...
void setupOpponentSensorsMenu(SensorConfig& sensorConfig) {
  ...
  const size_t maxVisibleItems = 8; // Maximum number of visible items on the screen
  ...
  while (true) {
    ...
    if (encoderHandler.isButtonPressed()) {
      ...
      selectedSensor = !selectedSensor; // Toggle the sensor state

   //   saveSensorConfigToEEPROM(sensorConfig); // Save updated configuration to EEPROM
    }
